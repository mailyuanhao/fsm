...
def parse_args():
    parse = argparse.ArgumentParser(
        description="make rangel file automatically")
    parse.add_argument(SRC_DEF, type=Path, nargs=1,
                       help="the json file used to define your fsm")
    parse.add_argument(DEST_DEF, nargs=1, type=Path,
                       help="the folder where to store the output files")
...
if __name__ == '__main__':
    ...
    with open(src) as fp:
        ...
        with open(dest_header_path, r"w") as header, open(dest_src_path, r"w") as src:
            ...
            print(fsm.Machine2Dot().show(machine))
